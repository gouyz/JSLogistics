...
class JSLTripOrderConmentVC: GYZBaseVC {
    
    /// 选择结果回调
    var resultBlock:(() -> Void)?
    
    ///txtView 提示文字
    let placeHolder = "您对我们的服务有什么意见或者建议，商品的有点或者美中不足的地方都可以说说"
    /// 选择的图片
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "出行评价"
    ...
    func setUpUI(){
        ...
        tagImgView.snp.makeConstraints { (make) in
            ...
            make.size.equalTo(CGSize.init(width: 15, height: 13))
        }
    ...
    lazy var goodImgView: UIImageView = UIImageView.init(image: UIImage.init(named: "Icon_trip_driver_tag"))
    ///
    ...
    lazy var tagImgView: UIImageView = UIImageView.init(image: UIImage.init(named: "icon_car_tag_default"))
    ///
    ...
    func requestPublishDynamic(){
        ...
        let paramDic: [String:Any] = ["content":content,"appoint_id":orderId,"user_id":userDefaults.string(forKey: "userId") ?? "","img":imgUrls,"driver_rank":ratingView.rating,"environment_rank":wlRatingView.rating,"service_rank":serviceRatingView.rating]
        
        GYZNetWork.requestNetwork("appointment/addComment", parameters: paramDic,  success: { (response) in
...
extension JSLTripOrderConmentVC : UIImagePickerControllerDelegate,UINavigationControllerDelegate
{
...
extension JSLTripOrderConmentVC : UITextViewDelegate,LHSAddPhotoViewDelegate
{
